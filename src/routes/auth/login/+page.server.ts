...
export const actions = {
	login: async ({ request, cookies }: RequestEvent) => {
		...
		//Generate session
		return await db
			.none(`INSERT INTO sessions (cookieId, user_id, expiration_date) VALUES($1,$2,$3)`, [
				cookieId,
				user.id,
				expiration_date,
			])
	...
	register: async ({ request, cookies, url, fetch }: RequestEvent) => {
		...
		const conflictUser = await db.any("SELECT email FROM users WHERE email=$1 OR name=$2", [
			email,
			username,
		]);
		...
		let id = await db.one(
			`INSERT INTO users(email, name, password, role, is_email_validated) 
			VALUES ($1, $2, $3, 'USER', FALSE) RETURNING id`,
			[email, username, await hash(password!, 15)],
			(a) => a.id,
		);
