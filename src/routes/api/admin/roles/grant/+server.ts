...
export const GET = (async ({ url, locals }) => {
	...
	if (userId && !hasRolePermission("GRANT_ROLE_" + (await getUserRole({ id: userId }))?.name, locals.user?.role)) {
		throw error(403)
	}
	...
	return db.one("SELECT id, role, member_start, member_stop, email FROM users WHERE id=$1;",
		...
		.then((user) => {
			...
			if (roleModified) {
				...
				if(body.role != Roles.MEMBER.name) db.none(`UPDATE users SET member_start=NULL, member_stop=NULL WHERE id=$1`, [user.id])
			}
