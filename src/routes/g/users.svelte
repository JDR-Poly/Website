...
<form on:submit|preventDefault={async () => {
    ...
    <div class="searchBar" on:focusout={(event) => {
        setTimeout(() => {
            selectedIndex = -1
            searchCompletion = []
        }, 100)
    }}>
        <input type="text" placeholder="Chercher un utilisateur" bind:value={searchText} on:input={inputChange} on:keydown={onKeypressInput} >
        <div class="searchBar-items">
            {#each searchCompletion as result, i}
                <a href="/u/profile/{result.id}"><div class="{i == selectedIndex ? 'searchBar-active' : ''}"> <strong>{result.name}</strong></div></a>
            {/each}
        </div>
    </div>
