...
export const actions = {
	...
	updatePassword: async ({ request, locals }) => {
		...
		try {
			...
			return await db
				.none(`UPDATE users SET password=$1 WHERE id=$2`, [newHashPassword, locals.user?.id])
				.then(() => {
					return { sucess: true };
				});
		} catch (err: any) {
	...
	updateName: async ({ request, locals }) => {
		...
		return await db
			.none(`UPDATE users SET name=$1 WHERE id=$2`, [name, locals.user?.id])
			.then(() => {
				return { sucess: true };
			})
			.catch((err) => {
	...
	updateEmail: async ({ request, locals, url }) => {
		...
		try {
			...
			return await db
				.none(`UPDATE users SET email=$1, is_email_validated=FALSE WHERE id=$2`, [
					email,
					locals.user?.id,
				])
				.then(() => {
	...
	deleteAccount: async ({ request, locals }) => {
		...
		try {
			...
			return await db
				.none(`DELETE FROM events WHERE author=$1; DELETE FROM users WHERE id=$1;`, [locals.user?.id])
				.then(() => {
					return { sucess: true };
				});
		} catch (err: any) {
