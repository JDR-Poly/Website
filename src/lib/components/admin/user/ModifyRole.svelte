<script lang="ts">
	...
	const rolesQuery = fetch(roleListUrl).then((res) =>
		res.ok ? res.json().then((json) => json.roles) : []
	);
	...
	{#if memberPeriod}
		...
		{:else}
			{#if roleName === Roles.USER.name && memberPeriod.start}
				...
				<p>
					? : Vous pouvez changer la fin et le début de la période de membre en selectionnant le
					rôle MEMBER
				</p>
			{/if}
			<br />
